...
class Decoder(Module):
    def __init__ (self, c_out,embed_size = 2, hidden_size=100,nf=64,kernel_size=3, n_layers=1, bias=True, rnn_dropout=0, bidirectional=False, init_weights=True):
        ...
        self.rnn1 = nn.LSTM(embed_size, hidden_size//2, num_layers=n_layers, bias=bias, batch_first=True, dropout=rnn_dropout, 
                              bidirectional=bidirectional)
        self.rnn2 = nn.LSTM(hidden_size//2, hidden_size, num_layers=n_layers, bias=bias, batch_first=True, dropout=rnn_dropout, 
                              bidirectional=bidirectional)
        self.rnn3 = nn.LSTM(hidden_size, c_out, num_layers=n_layers, bias=bias, batch_first=True, dropout=rnn_dropout, 
                              bidirectional=bidirectional)
...
class EncoderClassifier(Module):
    def __init__(self,c_in,c_out,encoder,n_neurons=20,fc_dropout=0,embed_size = 2):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
